...
from control_tower.incident_llm import (
    check_ollama_health,
    resolve_incident_llm_settings,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="List or apply incident recommendations."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Create/update incidents from recommendations.",
    )
    parser.add_argument(
        "--owner", default="auto-ops", help="Incident owner when --apply is used."
    )
    ...
    parser.add_argument(
        "--ollama-base-url",
        default=None,
        help="Ollama base URL (default: LP_OLLAMA_BASE_URL).",
    )
    parser.add_argument(
        "--ollama-model", default=None, help="Ollama model (default: LP_OLLAMA_MODEL)."
    )
    parser.add_argument(
        "--ollama-timeout-sec",
        type=float,
    ...
    if args.ollama_healthz or str(runtime["provider"]) == "ollama":
        ...
        if (
            not bool(health.get("ok"))
            and args.fail_on_llm_error
            and str(runtime["provider"]) == "ollama"
        ):
            raise RuntimeError(
                f"ollama health check failed: {health.get('error', 'unknown_error')}"
            )
