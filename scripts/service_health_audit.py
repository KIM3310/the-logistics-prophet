...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit service readiness and operational health contracts."
    )
    parser.add_argument("--max-pipeline-age-hours", type=float, default=24.0)
    parser.add_argument("--min-model-auc", type=float, default=0.71)
    parser.add_argument("--allow-queue-mismatch", action="store_true")
    parser.add_argument("--warn-as-error", action="store_true")
    parser.add_argument(
        "--json-out", default="", help="Optional path to write JSON report."
    )
