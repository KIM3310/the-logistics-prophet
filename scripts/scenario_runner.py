...
def _render_md(
    *,
    ...
    model_metrics = (
        metrics.get("model_test_metrics", {}) if isinstance(metrics, dict) else {}
    )
    quality_gate = metrics.get("quality_gate", {}) if isinstance(metrics, dict) else {}
    service_queue = (
        metrics.get("service_queue", {}) if isinstance(metrics, dict) else {}
    )
    ...
    q_status = str(
        quality_gate.get("status", quality.get("status", "unknown")) or "unknown"
    )
    ...
    lines.append(
        f"- Queue unresolved: **{service_queue.get('unresolved', queue_summary.get('unresolved', '-'))}**"
    )
    lines.append(
        f"- Critical open: **{service_queue.get('critical_open', queue_summary.get('critical_open', '-'))}**"
    )
    lines.append("")

    lines.append("## Quality Gate")
    lines.append(
        f"- Status: `{q_status}` (fails={quality_gate.get('fail_count', quality.get('fail_count', 0))}, warns={quality_gate.get('warn_count', quality.get('warn_count', 0))})"
    )
    ...
    stage_backlog = (
        core_snapshot.get("stage_backlog", [])
    ...
    if stage_backlog:
        ...
        for row in stage_backlog[:6]:
            lines.append(
                f"| {row.get('stage', '')} | {row.get('count', '')} | {row.get('share_pct', '')} |"
            )
        lines.append("")
    ...
    if isinstance(ops_health, dict):
        ...
        lines.append(
            f"- Critical unassigned: `{ops_health.get('critical_unassigned', '-')}`"
        )
        lines.append(
            f"- Avg unresolved age(h): `{ops_health.get('avg_unresolved_age_hours', '-')}`"
        )
    lines.append("")

    lines.append("## Governance")
    lines.append(
        f"- Audit chain valid: **{audit_ok}** (checked={audit.get('checked', '-')})"
    )
    lines.append(
        f"- Latest hash: `{str(audit.get('latest_hash', '') or '')[:18]}...`"
        if audit.get("latest_hash")
        else "- Latest hash: `-`"
    )
    open_incidents = [
        row
        for row in incidents
        if str(row.get("status", "")).lower() in {"open", "monitoring"}
    ]
    lines.append(
        f"- Open/Monitoring incidents: `{len(open_incidents)}` (total={len(incidents)})"
    )
    lines.append("")
    ...
    summary = (
        service_health.get("summary", {}) if isinstance(service_health, dict) else {}
    )
    lines.append(
        f"- pass={summary.get('pass_count', 0)} warn={summary.get('warn_count', 0)} fail={summary.get('fail_count', 0)}"
    )
    checks = (
        service_health.get("checks", []) if isinstance(service_health, dict) else []
    )
    if checks:
    ...
    if pipeline_status:
        ...
        if run_id or status:
            ...
            lines.append(
                f"- finished_at_utc: `{pipeline_status.get('finished_at_utc', '-')}`"
            )
            lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a repeatable end-to-end validation and export a report + evidence pack."
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force pipeline execution before exporting.",
    )
    parser.add_argument(
        "--out-dir",
        default="",
        help="Output directory (default: data/output/scenario_runs/<timestamp>).",
    )
    parser.add_argument(
        "--no-graph",
        action="store_true",
        help="Exclude RDF instance graph from evidence pack.",
    )
    parser.add_argument(
        "--no-ops-report",
        action="store_true",
        help="Exclude ops HTML report from evidence pack.",
    )
    ...
    out_dir = (
        ...
        else (ROOT / "data" / "output" / "scenario_runs" / started_at.replace(":", ""))
    )
    ...
    quality_status = str(
        metrics.get("quality_gate", {}).get("status", quality.get("status", "unknown"))
        or "unknown"
    )
    ...
    print(
        f"[scenario] pipeline_ready={verdict.pipeline_ready} quality={verdict.quality_status} audit_valid={verdict.audit_valid}"
    )
