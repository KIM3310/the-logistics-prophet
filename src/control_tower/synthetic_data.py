...
logger = logging.getLogger("control_tower.synthetic_data")
...
def _build_customers(
    rng: np.random.Generator, n_customers: int, start_date: date
) -> List[dict]:
...
def generate_synthetic_data(
    ...
    """Generate synthetic logistics data for the control tower pipeline."""
    logger.info(
        "Generating synthetic data: seed=%d, days=%d, orders_per_day=%d",
        seed,
        days,
        orders_per_day,
    )
    ...
    if start_date is None:
        ...
        if anchored:
            ...
            logger.info("Using anchored start date: %s", start_date)
        else:
            start_date = date.today() - timedelta(days=days)
            logger.info("Using computed start date: %s", start_date)
    ...
    for d in date_axis:
        ...
        for _ in range(daily_orders):
            ...
            warehouse_load_pct = float(
                np.round(np.clip(base_load + load_noise, 0.35, 0.98), 3)
            )

            peak_flag = 1 if seasonal_peak or weekly_peak else 0
            quantity = int(rng.integers(1, 5))
            order_value = float(
                np.round(quantity * product["price_usd"] * rng.uniform(0.9, 1.08), 2)
            )

            promised_days = int(
                np.clip(np.round(distance_km / 430 + rng.normal(1.2, 0.5)), 1, 8)
            )
            ...
            if delayed:
                primary_event = str(
                    rng.choice(["Weather", "Capacity", "Carrier", "Routing", "Customs"])
                )
                severity = str(
                    rng.choice(["Medium", "High", "Critical"], p=[0.40, 0.44, 0.16])
                )
                delay_events.append(
                    {
                ...
                if rng.random() < 0.28:
                    secondary_event = str(
                        rng.choice(["Capacity", "Carrier", "Routing"])
                    )
                    delay_events.append(
                        {
                            "event_id": f"E{event_seq:08d}",
    ...
    logger.info("Writing synthetic CSVs to %s", RAW_DIR)
    _write_csv(
    ...
    logger.info(
        "Synthetic data generated: orders=%d, shipments=%d, delay_events=%d",
        summary.orders,
        summary.shipments,
        summary.delay_events,
    )
    return summary
