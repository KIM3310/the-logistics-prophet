...
logger = logging.getLogger("control_tower.semantic_queries")

EX = Namespace("http://example.com/supply-chain#")


def shipment_iri(shipment_id: str) -> str:
    """Build a full IRI for a shipment identifier."""
...
def load_instance_graph(graph_path: Path) -> Graph:
    """Parse and return an RDF graph from a Turtle file."""
    logger.info("Loading instance graph from %s", graph_path)
    graph = Graph()
    graph.parse(str(graph_path), format="turtle")
    logger.info("Instance graph loaded: %d triples", len(graph))
    return graph


def query_shipment_evidence(
    graph: Graph, shipment_id: str, event_limit: int = 8
) -> ShipmentEvidence:
    """Query the RDF graph for full evidence about a specific shipment."""
    logger.info(
        "Querying shipment evidence for %s (event_limit=%d)", shipment_id, event_limit
    )
    ...
    logger.debug(
        "Shipment evidence for %s: carrier=%s, risk_band=%s, events=%d",
        shipment_id,
        carrier,
        risk_band,
        len(delay_events),
