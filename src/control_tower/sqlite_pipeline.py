...
logger = logging.getLogger("control_tower.sqlite_pipeline")
...
def build_sqlite_marts(
    sqlite_path: Path = SQLITE_PATH, raw_dir: Path = RAW_DIR
) -> dict:
    """Build the SQLite analytical marts from raw CSV files."""
    logger.info("Building SQLite marts: sqlite=%s, raw_dir=%s", sqlite_path, raw_dir)
    ...
    try:
        ...
        _create_tables(conn)
        logger.debug("Loading raw CSV data")
        _load_raw_tables(conn, raw_dir)
        logger.debug("Creating indexes")
        _create_indexes(conn)
        logger.debug("Building feature mart")
        _create_feature_mart(conn)
        logger.debug("Building KPI tables")
        ...
        logger.info("Mart row counts: %s", counts)
    except Exception:
        logger.error("SQLite mart build failed, cleaning up temp file %s", temp_path)
        if temp_path.exists():
            temp_path.unlink()
        raise
    finally:
    ...
    logger.info("SQLite marts built successfully at %s", sqlite_path)
    return counts
...
def _load_raw_tables(conn: sqlite3.Connection, raw_dir: Path) -> None:
    for table, columns in TABLE_COLUMNS.items():
        ...
        if not csv_path.exists():
            logger.error("Missing input CSV: %s", csv_path)
        ...
        logger.debug("Loaded table '%s' from %s", table, csv_path)
