...
logger = logging.getLogger("control_tower.service_store")
...
def _ensure_columns(
    conn: sqlite3.Connection, table: str, required: Dict[str, str]
) -> None:
...
def _safe_positive_int(
    value: object, default: int, *, min_value: int = 1, max_value: int = 10_000
) -> int:
    try:
...
def allowed_next_statuses(
    current_status: str, actor_role: str = "operator"
) -> List[str]:
...
def _validate_status_transition(
    current_status: str, next_status: str, actor_role: str
) -> None:
    ...
    if nxt not in allowed:
        allowed_labels = (
            ", ".join([status for status in STATUS_ORDER if status in allowed])
            if allowed
            else "(none)"
        )
        raise ValueError(
            f"invalid transition {current} -> {nxt}; allowed: {allowed_labels}"
        )
...
def _hash_password(password: str, salt_hex: str | None = None) -> Tuple[str, str]:
    ...
    digest = hashlib.pbkdf2_hmac(
        "sha256", password.encode("utf-8"), salt, 160_000
    ).hex()
...
def init_service_store(path: Path = SERVICE_DB_PATH) -> None:
    logger.debug("Initializing service store at %s", path)
    ...
    try:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_service_queue_owner ON service_queue(owner)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_service_queue_updated_at ON service_queue(updated_at)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_service_queue_eta_action_at ON service_queue(eta_action_at)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_service_incidents_source_rule ON service_incidents(source_rule)"
        )
        conn.execute(
...
def _ensure_default_users(conn: sqlite3.Connection) -> None:
    ...
    defaults = [
        (
            ...
            "Admin",
            "admin",
            _env_password("LP_DEMO_ADMIN_PASSWORD", "AdminOps!2026"),
        ),
        (
            "operator",
            "Operator",
            "operator",
            _env_password("LP_DEMO_OPERATOR_PASSWORD", "OperatorOps!2026"),
        ),
        (
            "viewer",
            "Viewer",
            "viewer",
            _env_password("LP_DEMO_VIEWER_PASSWORD", "ViewerOps!2026"),
        ),
    ]
    rows = []
...
def list_users(
    path: Path = SERVICE_DB_PATH, include_inactive: bool = False
...
def authenticate_user_with_status(
    username: str, password: str, path: Path = SERVICE_DB_PATH
) -> Dict[str, object]:
    ...
    try:
        ...
        if not _verify_password(
            password, str(row["password_hash"]), str(row["password_salt"])
        ):
...
def authenticate_user(
    ...
    result = authenticate_user_with_status(
...
def _append_activity(
    ...
    prev_hash_row = conn.execute(
        "SELECT event_hash FROM service_activity_log ORDER BY id DESC LIMIT 1"
    ).fetchone()
    prev_hash_candidate = (
        str(prev_hash_row[0]) if prev_hash_row and prev_hash_row[0] else ""
    )
...
def verify_audit_chain(
    path: Path = SERVICE_DB_PATH, limit: int = 5000
) -> Dict[str, object]:
    logger.info("Verifying audit chain at %s (limit=%d)", path, limit)
    init_service_store(path)
    ...
    for row in rows:
        ...
        if len(row_event_hash) != 64 or (
            row_prev_hash not in ("GENESIS", "") and len(row_prev_hash) != 64
        ):
            skipped_legacy += 1
            continue
...
def upsert_queue_rows(
    rows: List[Dict[str, object]], path: Path = SERVICE_DB_PATH
) -> int:
    logger.info("Upserting %d queue rows to %s", len(rows), path)
    init_service_store(path)
    if not rows:
    ...
    try:
        existing_ids = {
            str(r[0])
            for r in conn.execute("SELECT shipment_id FROM service_queue").fetchall()
        }
...
def fetch_ops_health(
    path: Path = SERVICE_DB_PATH, owner_limit: int = 8
) -> Dict[str, object]:
    ...
    avg_age = (
        round(sum(unresolved_age_hours) / len(unresolved_age_hours), 2)
        if unresolved_age_hours
        else 0.0
    )
    ...
    return {
        ...
        "owner_backlog": [
            {"owner": owner, "count": int(cnt)}
            for owner, cnt in owner_backlog.most_common(safe_owner_limit)
        ],
    }


def fetch_service_core_snapshot(
    path: Path = SERVICE_DB_PATH, candidate_limit: int = 12
) -> Dict[str, object]:
    safe_candidate_limit = _safe_positive_int(
        candidate_limit, 12, min_value=1, max_value=200
    )
    rows = fetch_queue(path=path, limit=10_000)
    now = datetime.now(timezone.utc)
    ...
    for row in rows:
        ...
        if status in UNRESOLVED_STATUSES:
            ...
            stale_hours = (
                round(((now - updated_dt).total_seconds() / 3600.0), 2)
                if updated_dt
                else 0.0
            )
        ...
        stage_backlog.append(
            {
                ...
                "count": int(count),
                "share_pct": round((float(count) / float(queue_size)) * 100.0, 1)
                if queue_size
                else 0.0,
                "stage_order": min(
                    [
                        int(
                            service_core_term_for_status(status).get("stage_order", 999)
                        )
    ...
    driver_hotspots = [
        {"driver": key, "count": int(cnt)} for key, cnt in driver_counts.most_common(8)
    ]
...
def fetch_workflow_sla_snapshot(
    path: Path = SERVICE_DB_PATH, candidate_limit: int = 12
) -> Dict[str, object]:
    safe_candidate_limit = _safe_positive_int(
        candidate_limit, 12, min_value=1, max_value=200
    ...
    stage_counters: Dict[str, Dict[str, object]] = {
        status: {
            "status": status,
            "core_stage": service_core_term_for_status(status).get(
                "core_label", "Unknown"
            ),
    ...
    for row in rows:
        ...
        if threshold is not None:
            stage_counters[status]["in_stage"] = (
                int(stage_counters[status]["in_stage"]) + 1
            )
            if age_hours >= threshold:
                stage_counters[status]["breached"] = (
                    int(stage_counters[status]["breached"]) + 1
                )
                breached_total += 1
                risk_score = float(row.get("risk_score", 0.0) or 0.0)
                risk_band = str(row.get("risk_band", "Low"))
                core_stage = str(
                    service_core_term_for_status(status).get("core_label", "Unknown")
                )
    ...
    breach_rate = (
        round((float(breached_total) / float(unresolved_total)) * 100.0, 1)
        if unresolved_total
        else 0.0
    )
    ...
    return {
        ...
        "age_buckets": [
            {"bucket": bucket, "count": int(cnt)} for bucket, cnt in age_buckets.items()
        ],
        "stage_sla": [
            {
                **item,
...
def fetch_service_core_worklist(
    path: Path = SERVICE_DB_PATH, per_stage_limit: int = 6
) -> Dict[str, object]:
    safe_per_stage_limit = _safe_positive_int(
        per_stage_limit, 6, min_value=1, max_value=100
    )
    rows = fetch_queue(path=path, limit=10_000)
    now = datetime.now(timezone.utc)

    stage_order = ["Start", "Check", "Fix"]
    stage_items: Dict[str, List[Dict[str, object]]] = {
        ...
        stage_items[stage].append(
            {
                ...
                "next_step": _next_step_hint(
                    status=status, no_owner=no_owner, overdue_eta=overdue_eta
                ),
                "why": ", ".join(why),
            }
        )
        ...
        stages.append(
            {
                ...
                "label": SERVICE_CORE_STAGE_LABELS.get(stage, stage),
                "count": len(items),
                "share_pct": round((float(len(items)) / float(total_open)) * 100.0, 1)
                if total_open
                else 0.0,
    ...
    for stage_entry in stages:
        stage_items_list = stage_entry.get("items", [])
        if isinstance(stage_items_list, list):
...
def update_queue_action(
    ...
    logger.info(
        "Updating queue action: shipment=%s, status=%s, owner=%s, actor=%s",
        shipment_id,
        status,
        owner,
        actor,
    ...
    try:
        ...
        _validate_status_transition(
            str(prev["status"]), normalized_status, actor_role=actor_role
        )
...
def bulk_update_queue_actions(
    ...
    try:
        ...
        for shipment_id in normalized_ids:
            ...
            try:
                _validate_status_transition(
                    current_status, normalized_status, actor_role=actor_role
                )
        ...
        missing = [
            shipment_id for shipment_id in normalized_ids if shipment_id not in curr_map
        ]
        invalid_ids = {str(item.get("shipment_id", "")) for item in invalid_transitions}
        updated_ids = [
            shipment_id
            for shipment_id in normalized_ids
            if shipment_id in curr_map and shipment_id not in invalid_ids
        ]

        previous_state = {
            shipment_id: prev_map.get(shipment_id, {}) for shipment_id in updated_ids
        }
        new_state = {
            shipment_id: curr_map.get(shipment_id, {}) for shipment_id in updated_ids
        }
        _append_activity(
            conn=conn,
            actor=actor,
...
def fetch_activity(
    shipment_id: str, path: Path = SERVICE_DB_PATH, limit: int = 30
) -> List[Dict[str, object]]:
    init_service_store(path)
...
def list_recent_activity(
    path: Path = SERVICE_DB_PATH, limit: int = 100
) -> List[Dict[str, object]]:
    init_service_store(path)
...
def upsert_incident(
    ...
    logger.info(
        "Upserting incident: severity=%s, status=%s, actor=%s, rule=%s",
        severity,
        status,
        actor,
        source_rule,
    ...
    if not has_permission(actor_role, "incident_manage"):
        logger.warning(
            "Permission denied: actor=%s role=%s lacks incident_manage",
            actor,
            actor_role,
        )
    ...
    if not title:
        logger.error("Incident title is empty")
        raise ValueError("incident title is required")
    if severity not in INCIDENT_SEVERITIES:
        logger.error(
            "Invalid incident severity: %s (allowed: %s)", severity, INCIDENT_SEVERITIES
        )
        raise ValueError(f"invalid severity: {severity}")
    if status not in INCIDENT_STATUSES:
        logger.error(
            "Invalid incident status: %s (allowed: %s)", status, INCIDENT_STATUSES
        )
...
def upsert_incident_from_recommendation(
    ...
    if not description:
        description = (
            f"Rule-based recommendation generated from {rule_id or 'unspecified rule'}."
        )
...
def list_incidents(
    path: Path = SERVICE_DB_PATH, limit: int = 50
) -> List[Dict[str, object]]:
    init_service_store(path)
...
def log_pipeline_run(
    ...
    logger.info(
        "Logging pipeline run: run_id=%s, status=%s, steps=%d, duration=%.2fs",
        run_id,
        status,
        step_count,
        duration_sec,
...
def list_pipeline_runs(
    path: Path = SERVICE_DB_PATH, limit: int = 30
) -> List[Dict[str, object]]:
    init_service_store(path)
    safe_limit = _safe_positive_int(limit, 30, min_value=1, max_value=500)
    conn = sqlite3.connect(path)
