...
logger = logging.getLogger("control_tower.semantic_layer")
...
def build_instance_graph(
    ...
    """Build an RDF instance graph from the SQLite database and risk queue."""
    logger.info(
        "Building instance graph from %s (limit=%d)", sqlite_path, shipment_limit
    )
    ...
    try:
        carriers = conn.execute(
            "SELECT carrier_id, reliability_score FROM carriers"
        ).fetchall()
        for row in carriers:
            ...
            g.add(
                (
                    carrier,
                    EX.reliabilityScore,
                    Literal(float(row["reliability_score"]), datatype=XSD.decimal),
                )
            )
        logger.debug("Added %d carriers to graph", len(carriers))
        ...
        customers = conn.execute(
            "SELECT customer_id FROM customers LIMIT 5000"
        ).fetchall()
        ...
        for row in shipments:
            ...
            g.add(
                (
                    order,
                    EX.orderDate,
                    Literal(str(row["order_date"]), datatype=XSD.date),
                )
            )
            g.add(
                (
                    order,
                    EX.promisedDays,
                    Literal(int(float(row["promised_days"])), datatype=XSD.integer),
                )
            )
            g.add(
                (
                    order,
                    EX.orderValueUsd,
                    Literal(float(row["order_value_usd"]), datatype=XSD.decimal),
                )
            )
            g.add(
                (
                    shipment,
                    EX.distanceKm,
                    Literal(float(row["distance_km"]), datatype=XSD.decimal),
                )
            )
            g.add(
                (
                    shipment,
                    EX.weatherSeverity,
                    Literal(float(row["weather_severity"]), datatype=XSD.decimal),
                )
            )
            g.add(
                (
                    shipment,
                    EX.warehouseLoadPct,
                    Literal(float(row["warehouse_load_pct"]), datatype=XSD.decimal),
                )
            )
            g.add(
                (
                    ...
                    EX.deliveredOnTime,
                    Literal(
                        bool(1 - int(float(row["delivered_late"]))),
            ...
            g.add(
                (
                    shipment,
                    EX.delayHours,
                    Literal(float(row["delay_hours"]), datatype=XSD.decimal),
                )
            )
        ...
        logger.info("Materialized %d shipments into RDF graph", len(shipments))
        ...
        for row in delay_events:
            ...
            g.add(
                (
                    event,
                    EX.delayCause,
                    Literal(str(row["event_type"]), datatype=XSD.string),
                )
            )
            g.add(
                (event, EX.severity, Literal(str(row["severity"]), datatype=XSD.string))
            )
            g.add((shipment, EX.hasDelayEvent, event))

        logger.debug("Added %d delay events to graph", len(delay_events))
        ...
        if risk_queue_path.exists():
            ...
            with risk_queue_path.open("r", newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    risk_count += 1
            logger.debug("Added %d risk states from queue", risk_count)
        else:
            logger.debug(
                "Risk queue file not found, skipping risk states: %s", risk_queue_path
            )
    ...
    if ontology_path.exists():
        logger.debug("Parsing ontology from %s", ontology_path)
        g.parse(ontology_path, format="turtle")

    output_path.parent.mkdir(parents=True, exist_ok=True)
    g.serialize(destination=str(output_path), format="turtle")

    logger.info("Instance graph written: %d triples to %s", len(g), output_path)
    return {
        "triples": len(g),
...
def _query_to_dicts(
    graph: Graph, query: str, key_order: List[str]
...
def run_competency_queries(
    ...
    """Run SPARQL competency queries against the instance graph."""
    logger.info("Running competency queries on %s", graph_path)
    graph = Graph()
    ...
    for q in queries:
        logger.debug("Running query: %s", q["id"])
        ...
        logger.debug("Query %s returned %d rows", q["id"], len(rows))
    ...
    logger.info(
        "Competency queries complete: %d queries, results written to %s",
        len(result_queries),
        output_path,
    )
    return payload
...
def load_sparql_results(path: Path = SPARQL_RESULTS_PATH) -> Dict[str, object]:
    """Load previously saved SPARQL results from JSON."""
    if not path.exists():
        logger.debug("SPARQL results file not found: %s", path)
        return {"queries": []}
