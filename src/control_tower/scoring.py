...
logger = logging.getLogger("control_tower.scoring")
...
def _top_drivers(
    feature_names: List[str], contrib: np.ndarray, top_k: int = 3
) -> List[str]:
...
def score_latest_batch(
    ...
    """Score a batch of shipments and write the risk queue."""
    logger.info("Scoring batch: %d shipments, top_n=%d", len(batch_frame), top_n)
    ...
    logger.info("Risk queue written: %d items to %s", len(selected), output_path)

    critical_count = sum(1 for r in selected if r["risk_band"] == "Critical")
    high_count = sum(1 for r in selected if r["risk_band"] == "High")
    logger.info(
        "Risk distribution: critical=%d, high=%d, medium+low=%d",
        critical_count,
        high_count,
        len(selected) - critical_count - high_count,
    )

    upsert_queue_rows(selected, path=service_db_path)
    logger.info("Queue synced to service store at %s", service_db_path)
    return selected
