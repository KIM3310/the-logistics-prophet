...
logger = logging.getLogger("control_tower.ops_output")
...
def write_training_summary(
    ...
    """Write a human-readable training summary JSON."""
    logger.info("Writing training summary to %s", output_path)
    ...
    logger.info("Training summary written: selected_model=%s", selected)
    return summary
...
def write_monitoring_payload(
    ...
    """Build and write the monitoring metrics payload."""
    logger.info("Writing monitoring payload to %s", output_path)
    ...
    payload = {
        ...
        "risk_queue_metrics": {
            ...
            "critical_ratio": round(critical_count / len(ranked_queue), 4)
            if ranked_queue
            else 0.0,
        },
        "quality_gate": {
        ...
        "service_health": {
            ...
            "fail_count": ((service_health or {}).get("summary", {}) or {}).get(
                "fail_count", 0
            ),
            "warn_count": ((service_health or {}).get("summary", {}) or {}).get(
                "warn_count", 0
            ),
        },
    }
    ...
    logger.info(
        "Monitoring payload written: critical=%d, high=%d", critical_count, high_count
    )
    return payload
...
def build_ops_html_report(
    ...
    """Generate an HTML operations report."""
    logger.info("Building ops HTML report to %s", output_path)
    ...
    cause_rows = "\n".join(
        [
            f"<tr><td>{c.get('event_type', '')}</td><td>{c.get('cnt', '')}</td></tr>"
            for c in causes
        ]
    )
...
"""
    ...
    with output_path.open("w", encoding="utf-8") as f:
        f.write(html.strip() + "\n")
    logger.info("Ops HTML report written to %s", output_path)
