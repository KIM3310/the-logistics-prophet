...
logger = logging.getLogger("control_tower.service_health")

STATUS_ORDER = {"pass": 0, "warn": 1, "fail": 2}

# Health-check thresholds configurable via environment variables.
DEFAULT_MAX_PIPELINE_AGE_HOURS = float(
    os.getenv("HEALTH_MAX_PIPELINE_AGE_HOURS", "24.0")
)
DEFAULT_MIN_MODEL_AUC = float(os.getenv("HEALTH_MIN_MODEL_AUC", "0.71"))
DEFAULT_STRICT_QUEUE_PARITY = os.getenv(
    "HEALTH_STRICT_QUEUE_PARITY", "true"
).lower() in ("true", "1", "yes")
...
def _build_health_diagnostics(checks: List[Dict[str, Any]]) -> Dict[str, Any]:
    failing_check_ids = [
        str(item.get("id", "")) for item in checks if item.get("status") == "fail"
    ]
    warning_check_ids = [
        str(item.get("id", "")) for item in checks if item.get("status") == "warn"
    ]
    ...
    if failing_check_ids:
        ...
        next_action = (
            f"Review {warning_check_ids[0]} and rerun `make health` before release."
        )
    else:
...
def _build_review_summary(
    *,
    ...
    return {
        ...
        "headline": "Compact verification snapshot for queue parity, audit integrity, and operational actionability.",
        "summary": {
...
def _build_recovery_drill(
    *,
    ...
    recovered_risk = max(
        20,
        baseline_risk
        - min(18, 6 + warn_count * 2 + (0 if queue_csv_rows == db_queue_count else 6)),
    )
...
def _build_decision_board(
    *,
    ...
    queue_check = next(
        (item for item in checks if item.get("id") == "queue_parity"), {}
    )
        ...
        items.append(
            {
                ...
                "expected_delta": {
                    "queue_mismatch_rows": abs(
                        int(queue_csv_rows) - int(db_queue_count)
                    ),
    ...
    return {
        ...
        "review_actions": [
            ...
            "Pair every recommended action with the output flow that justifies it.",
...
def _build_action_impact_board(
    *,
    ...
    recovery_summary = (
        recovery_drill.get("summary", {}) if isinstance(recovery_drill, dict) else {}
    )
    recovery_items = [
    ...
    action_rows = [
        {
            ...
            "owner": owner_map.get(
                str(item.get("lane", "steady-state")), "ops-control"
            ),
    ...
    return {
        ...
        "kpis": [
            ...
            },
            {
                ...
                "baseline": 55
                if action_rows and action_rows[0]["lane"] != "steady-state"
                else 72,
                "expected_after_actions": 78
                if action_rows and action_rows[0]["lane"] != "steady-state"
                else 88,
                "delta": 23
                if action_rows and action_rows[0]["lane"] != "steady-state"
...
def _build_compact_summary(
    *,
    ...
    decision_board: Dict[str, Any],
    action_impact_board: Dict[str, Any],
) -> Dict[str, Any]:
    review_meta = review_summary.get("summary", {}) if isinstance(review_summary, dict) else {}
    runtime_meta = runtime_scorecard.get("summary", {}) if isinstance(runtime_scorecard, dict) else {}
    decision_meta = decision_board.get("summary", {}) if isinstance(decision_board, dict) else {}
    action_meta = action_impact_board.get("summary", {}) if isinstance(action_impact_board, dict) else {}

    return {
        "contract": "logistics-control-compact-summary-v1",
        "headline": "Compact proof summary for health posture, decision pressure, and expected operator impact.",
        "summary": {
            "runtime_score": int(runtime_meta.get("runtime_score", 0)),
            "queue_csv_rows": int(review_meta.get("queue_csv_rows", 0)),
            "audit_chain_checked": int(review_meta.get("audit_chain_checked", 0)),
            "recommended_actions": int(decision_meta.get("recommended_actions", 0)),
            "eta_gain_hours": int(action_meta.get("eta_gain_hours", 0)),
        },
        "reviewer_fast_path": [
            "make health",
            "make compact-summary",
            "make review-pack",
            "make decision-board",
            "make action-impact",
        ],
        "route_bundle": {
            "health": "make health",
            "compact_summary": "make compact-summary",
            "review_summary": "make review-summary",
            "review_pack": "make review-pack",
            "decision_board": "make decision-board",
            "action_impact": "make action-impact",
        },
        "claims": [
            "Queue parity and audit integrity stay visible before operator action is recommended.",
            "Decision pressure and expected KPI impact are shown together instead of in separate reviewer steps.",
            "This summary is still anchored to local synthetic evidence, not live carrier systems.",
        ],
    }
...
def _queue_csv_summary(path: Path) -> Dict[str, int]:
    ...
    shipment_ids = [
        ...
        for row in rows
        if str(row.get("shipment_id", "")).strip()
    ]
...
def build_service_health_report(
    *,
    ...
    max_pipeline_age_hours: float = DEFAULT_MAX_PIPELINE_AGE_HOURS,
    min_model_auc: float = DEFAULT_MIN_MODEL_AUC,
    strict_queue_parity: bool = DEFAULT_STRICT_QUEUE_PARITY,
    now_utc: datetime | None = None,
) -> Dict[str, Any]:
    logger.info(
        "Building service health report (max_age=%.1fh, min_auc=%.4f, strict_parity=%s)",
        max_pipeline_age_hours,
        min_model_auc,
        strict_queue_parity,
    )
        ...
        checks.append(
            {
                "id": "pipeline_status",
                "status": "fail",
                "detail": "pipeline status file missing",
            }
        )
        checks.append(
            {
            ...
            checks.append(
                {
                    "id": "pipeline_freshness",
                    "status": "fail",
                    "detail": "finished_at_utc missing/invalid",
                }
            )
        else:
    ...
    if not quality:
        checks.append(
            {"id": "quality_gate", "status": "fail", "detail": "quality report missing"}
        )
    ...
    model_auc = _safe_float(
        (training.get("test_metrics", {}) or {}).get("auc"), default=-1.0
    )
        ...
        checks.append(
            {
                "id": "model_auc",
                "status": "fail",
                "detail": "training summary missing/invalid auc",
            }
        )
        ...
        checks.append(
            {
                "id": "queue_parity",
                "status": "fail",
                "detail": "daily_risk_queue.csv missing",
            }
        )
        ...
        checks.append(
            {
                "id": "queue_parity",
                "status": "fail",
                "detail": "service_store.db missing",
            }
        )
    else:
        if duplicate_rows > 0:
            ...
            checks.append(
                {
                    "id": "queue_duplicates",
                    "status": "pass",
                    "detail": "no duplicate shipment rows",
                }
            )
    ...
    if not service_db_path.exists():
        checks.append(
            {"id": "audit_chain", "status": "fail", "detail": "service db missing"}
        )
    else:
        ...
        "service_meta": {
            ...
            "proof_assets": [
                ...
                },
                {
                    ...
                    "path": "app/dashboard.py?page=next-actions",
                    "kind": "surface",
                    "why": "Makes the expected KPI delta explicit before operators or operators accept the recommendation.",
                },
                {
                    "label": "Evidence Pack",
            ...
            "review_pack": {
                ...
                "headline": "Verification pack for the logistics worklist: queue parity, audit chain, action loop, and evidence exports in one surface.",
                "proof_bundle": {
                    "queue_csv_rows": int(queue_csv.get("row_count", 0)),
                    "service_db_rows": db_queue_count,
                    "strict_queue_parity": bool(strict_queue_parity),
                ...
                "trust_boundary": [
                    ...
                    "Datadog exports and Ollama recommendations remain optional optional extensions rather than baseline runtime requirements.",
                ...
                "two_minute_review": [
                    ...
                    "Open Governance or Evidence Pack before exporting export artifacts.",
                ],
                "proof_assets": [
                    {
                        "label": "Health Audit",
                        "path": "make health",
                        "kind": "command",
                        "why": "Verifies the baseline pipeline and audit posture before any output claims are made.",
                    },
                    {
                        "label": "Control Tower",
                        "path": "app/dashboard.py?page=control-tower",
...
def health_exit_code(report: Dict[str, Any], warn_as_error: bool = False) -> int:
    ...
    if overall == "fail":
        logger.warning("Health check exit code 1: overall status is fail")
        return 1
    if overall == "warn" and warn_as_error:
        logger.warning(
            "Health check exit code 2: overall status is warn (warn_as_error=True)"
        )
        return 2
    logger.info("Health check exit code 0: overall status is %s", overall)
    return 0
