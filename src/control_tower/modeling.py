...
logger = logging.getLogger("control_tower.modeling")
...
def optimize_threshold(y_true: np.ndarray, y_prob: np.ndarray) -> float:
    ...
    for t in np.linspace(0.30, 0.80, 51):
        ...
        if score > best_f1:
            best_f1 = score
            best_t = float(t)
    logger.debug("Optimized threshold: %.3f (F1=%.4f)", best_t, best_f1)
...
def train_model_suite(
    ...
    """Train baseline and challenger models, select the best, and return artifact + bundle."""
    logger.info(
        "Training model suite: %d train rows, %d test rows, %d features",
        len(train_df),
        len(test_df),
        len(feature_names),
    )
    ...
    logger.info("Training challenger HistGradientBoosting model")
    ...
    logger.info("Calibrating challenger model with isotonic regression (cv=3)")
    ...
    baseline_train_metrics = compute_metrics(
        y_train, baseline_train_prob, baseline_threshold
    )
    baseline_test_metrics = compute_metrics(
        y_test, baseline_test_prob, baseline_threshold
    )

    challenger_train_metrics = compute_metrics(
        y_train, challenger_train_prob, challenger_threshold
    )
    challenger_test_metrics = compute_metrics(
        y_test, challenger_test_prob, challenger_threshold
    )

    logger.info(
        "Baseline test AUC=%.4f, Challenger test AUC=%.4f",
        baseline_test_metrics["auc"],
        challenger_test_metrics["auc"],
    )

    if challenger_test_metrics["auc"] > baseline_test_metrics["auc"] or (
        ...
        selection_reason = (
            "Selected challenger by higher test AUC (tie-breaker: lower Brier)."
        )
    else:
        ...
        selection_reason = (
            "Selected baseline by higher or equal generalization quality."
        )

    logger.info(
        "Selected model: %s (threshold=%.3f). Reason: %s",
        selected_name,
        selected_threshold,
        selection_reason,
    )
...
def save_model_bundle(path: Path, bundle: Dict[str, object]) -> None:
    """Persist the model bundle via joblib."""
    path.parent.mkdir(parents=True, exist_ok=True)
...
def predict_scores(bundle: Dict[str, object], x: np.ndarray) -> np.ndarray:
    """Generate risk probability scores from the selected model."""
    model = bundle["selected_model"]
    prob = model.predict_proba(x)[:, 1]
    logger.debug("Predicted scores for %d samples", len(prob))
    return prob
...
def _tree_shap_values(model: object, x: np.ndarray) -> np.ndarray:
    ...
    logger.debug("Computing TreeExplainer SHAP values for %d samples", x.shape[0])
...
def _linear_contributions(model: Pipeline, x: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        logger.debug("Computing LinearExplainer SHAP values for %d samples", x.shape[0])
        ...
        logger.warning(
            "LinearExplainer failed, falling back to coefficient-based contributions"
        )
...
def write_shap_reports(
    ...
    """Write global and local SHAP contribution reports to CSV."""
    logger.info(
        "Writing SHAP reports: global=%s, local=%s",
        output_global_path,
        output_local_path,
    )
    ...
    logger.info(
        "SHAP reports written: global top=%s, local rows=%d",
        global_rows[0]["feature"] if global_rows else "N/A",
        len(local_rows),
    )
    return {
