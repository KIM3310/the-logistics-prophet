...
logger = logging.getLogger("control_tower.quality")
...
def _check_raw_schema(raw_dir: Path) -> List[QualityCheck]:
    logger.info("Checking raw CSV schemas in %s", raw_dir)
    ...
    for filename, expected in EXPECTED_RAW_COLUMNS.items():
        ...
        if not path.exists():
            ...
            checks.append(
                QualityCheck(
                    name=f"raw_schema:{filename}", status="fail", detail="file missing"
                )
            )
            continue
        ...
        if header != expected:
            logger.warning(
                "Schema mismatch in %s: expected=%s, got=%s", filename, expected, header
            )
            checks.append(
                QualityCheck(
            ...
            checks.append(
                QualityCheck(name=f"raw_schema:{filename}", status="pass", detail="ok")
            )
    return checks


def _check_sql_integrity(sqlite_path: Path) -> List[QualityCheck]:
    logger.info("Running SQL integrity checks on %s", sqlite_path)
    checks: List[QualityCheck] = []
    ...
    try:
        ...
        logger.info("model_features row count: %d", row_count)

        null_checks = []
        for col in FEATURE_COLUMNS + [TARGET_COLUMN]:
            n = conn.execute(
                f"SELECT COUNT(*) FROM model_features WHERE {col} IS NULL"
            ).fetchone()[0]
            null_checks.append((col, n))
        null_fail = [f"{col}:{n}" for col, n in null_checks if n > 0]
        if null_fail:
            logger.warning("Null values detected: %s", null_fail)
        checks.append(
            QualityCheck(
        ...
        if fk_failures:
            logger.warning("Referential integrity failures: %s", fk_failures)
        checks.append(
            QualityCheck(
        ...
        if range_issues:
            logger.warning("Range check issues: %s", range_issues)
        checks.append(
            QualityCheck(
                name="range_checks",
        ...
        if drift_status != "pass":
            logger.warning(
                "Distribution drift detected (%s): %s", drift_status, drift_notes
            )
        checks.append(
            QualityCheck(
...
def run_data_quality_checks(
    ...
    """Run the full data quality gate and write the report."""
    logger.info(
        "Running data quality checks: raw_dir=%s, sqlite=%s", raw_dir, sqlite_path
    )
    checks: List[QualityCheck] = []
    ...
    if sqlite_path.exists():
        ...
        logger.error("SQLite database missing: %s", sqlite_path)
        checks.append(
            QualityCheck(
                name="sqlite_presence", status="fail", detail=f"missing {sqlite_path}"
            )
        )
    ...
    logger.info(
        "Data quality report: status=%s, pass=%d, warn=%d, fail=%d",
        worst,
        summary["pass_count"],
        summary["warn_count"],
        summary["fail_count"],
