...
.hero h1 {
  font-family: ui-serif, Georgia, "Iowan Old Style", "Palatino Linotype", "Book Antiqua", Palatino, serif;
  margin: 0;
...
.risk-table tr.selected td {
  background: rgba(15,98,91,.06);
}

.risk-table tr.selected td:first-child {
  border-left: 4px solid rgba(15,98,91,.75);
}
...
div[data-testid="stDownloadButton"] > button:hover {
  box-shadow: 0 14px 24px rgba(44,35,25,0.12);
...
def render_ops_health_panel(service_summary: Dict[str, object], ops_health: Dict[str, object]) -> None:
    st.markdown('<section class="panel"><h3>Ops Health</h3>', unsafe_allow_html=True)
...
def render_quality_details_panel(quality: Dict[str, object]) -> None:
    st.markdown('<section class="panel"><h3>Quality Gate Details</h3>', unsafe_allow_html=True)
...
def render_model_selection_panel(training: Dict[str, object], model_comparison: Dict[str, object]) -> None:
    st.markdown('<section class="panel"><h3>Model Selection</h3>', unsafe_allow_html=True)
...
def render_queue_panel(queue: pd.DataFrame) -> pd.DataFrame:
    ...
    rows_html = []
    selected_id = get_selected_shipment(filtered)
    for _, row in filtered.head(top_n).iterrows():
        shipment_id = str(row.get("shipment_id", "") or "")
        row_class = "selected" if shipment_id == selected_id else ""
        rows_html.append(
...
<section class="panel">
  ...
  <table class="risk-table">
    ...
    # The HTML table is for presentation; selection happens via this control.
    ...
    if inspect_options:
        ...
        inspected = st.selectbox("Inspect Shipment", options=inspect_options, index=inspect_idx, key="queue_inspect")
...
def render_postmortem_export_panel(user: Dict[str, object]) -> None:
    st.markdown('<section class="panel"><h3>Postmortem Export</h3>', unsafe_allow_html=True)
...
def render_evidence_pack_panel(user: Dict[str, object]) -> None:
    st.markdown('<section class="panel"><h3>Evidence Pack</h3>', unsafe_allow_html=True)
    st.caption("Build a reviewer-friendly ZIP with key artifacts + integrity manifest (offline).")
...
def render_governance_overview_panel() -> None:
    st.markdown('<section class="panel"><h3>Governance</h3>', unsafe_allow_html=True)
...
def render_admin_panel(user: Dict[str, object]) -> None:
    ...
    st.markdown('<section class="panel"><h3>Admin: Users</h3>', unsafe_allow_html=True)
