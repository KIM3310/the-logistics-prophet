...
def ui_mode() -> str:
    ...
    UI modes:
      ...
      - cinematic: safe + small CSS layer with aurora glow effect
...
def render_nav(mode: str) -> str:
    ...
    if mode == "plain":
        st.caption(
            "Plain mode is enabled (`?ui=plain`). Visual polish is reduced for troubleshooting."
        )
    return str(page)
...
def load_shap_global(path: Path) -> pd.DataFrame:
    ...
    if "mean_abs_contribution" in frame.columns:
        frame["mean_abs_contribution"] = pd.to_numeric(
            frame["mean_abs_contribution"], errors="coerce"
        ).fillna(0.0)
...
def load_service_queue() -> pd.DataFrame:
    ...
    frame["risk_score"] = pd.to_numeric(frame["risk_score"], errors="coerce").fillna(
        0.0
    )
...
def reason_display(raw: str) -> str:
    ...
    if not tokens:
        return ""
    return ", ".join(
        REASON_TEXT.get(token, token.replace("_", " ")) for token in tokens
    )
...
def render_login_gate() -> Dict[str, object]:
    ...
    st.caption(
        "Secure access. Role-based permissions are enabled and actions are audit-tracked."
    )
    st.caption(
        "If the page looks blank, try `?ui=plain` or run `make demo-local-debug`."
    )
    ...
    if _env_flag("LP_SHOW_DEMO_CREDENTIALS", default=False):
        ...
        st.caption(
            "Demo credential hints are hidden. Set `LP_SHOW_DEMO_CREDENTIALS=1` to display."
        )
    ...
    if submitted:
        ...
        if not username_value or not password:
            ...
            auth_result = authenticate_user_with_status(
                username_value, password, path=SERVICE_DB_PATH
            )
...
def render_session_bar(
    ...
    pipeline_status: Dict[str, object] | None = None,
    metrics: Dict[str, object] | None = None,
) -> None:
    c1, c2, c3, c4, c5 = st.columns([2, 1, 1.25, 1, 1])
    with c1:
        st.caption(
            f"Signed in as `{user.get('display_name')} ({user.get('username')})`"
        )
    with c2:
...
def build_shap_chart(frame: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(
            showgrid=True, gridcolor="rgba(0,0,0,.08)", title="Mean |Contribution|"
        ),
...
def render_hero(
    ...
    *,
    ...
    with st.container(border=True):
        ...
        with r1[0]:
            st.metric(
                "On-Time %", f"{on_time:.1f}%", delta=f"{delta_on_time:+.1f}% vs target"
            )
        with r1[1]:
...
def render_analysis_panel(
    kpi_series: pd.DataFrame, shap_global: pd.DataFrame, quality: Dict[str, object]
) -> None:
    c1, c2 = st.columns([1.4, 1])
    with c1:
        with panel("Trend Over Time"):
            st.plotly_chart(
                build_kpi_chart(kpi_series),
        ...
        with c1:
            ...
            if not driver_df.empty:
                ...
                if "driver" in driver_df.columns:
                    driver_df["driver"] = driver_df["driver"].map(
                        lambda x: driver_display(x)
                    )
                st.dataframe(
        ...
        with c2:
            ...
            if not escalation_df.empty:
                if "status" in escalation_df.columns:
                    escalation_df["status"] = escalation_df["status"].map(
                        lambda x: status_display(str(x))
                    )
                if "reasons" in escalation_df.columns:
                    escalation_df["reasons"] = escalation_df["reasons"].map(
                        lambda x: reason_display(str(x))
                    )
                show_cols = [
                    "shipment_id",
                    "risk_band",
...
def render_service_core_worklist(
    worklist: Dict[str, object], user: Dict[str, object], queue: pd.DataFrame
) -> None:
    with panel(
        "Next Actions", caption="Top items for each step. Higher score = do first."
    ):
        stages = worklist.get("stages", []) if isinstance(worklist, dict) else []
        stage_map = {
            str(item.get("stage")): item for item in stages if isinstance(item, dict)
        }
        ...
        for idx, stage in enumerate(["Start", "Check", "Fix"]):
            with cols[idx]:
                ...
                item_lookup = {
                    ...
                    and _clean_text(item.get("shipment_id", ""))
                }
                ...
                shipment_choices = [
                    shipment_id for shipment_id in item_lookup.keys() if shipment_id
                ]
                if not shipment_choices:
                    continue

                selected = st.selectbox(
                    "Pick Item", options=shipment_choices, key=f"core_pick_{stage}"
                )
                ...
                current_status = (
                    _clean_text(current.get("status", ""))
                    or _clean_text(source.get("status", "New"))
                    or "New"
                )
                current_owner = _clean_text(current.get("owner", "")) or _clean_text(
                    source.get("owner", "")
                )
                current_note = _clean_text(current.get("note", ""))
                current_eta = _clean_text(
                    current.get("eta_action_at", "")
                ) or _clean_text(source.get("eta_action_at", ""))
                next_status = next_status_for_quick_move(
                    current_status=current_status, actor_role=role
                )
                ...
                if move:
                    try:
                        ...
                        st.success(
                            f"Moved {selected} to {status_display(next_status)}."
                        )
                        st.rerun()
                    except (ValueError, PermissionError) as exc:
                        st.error(str(exc))
...
def render_workflow_sla_panel(sla_snapshot: Dict[str, object]) -> None:
    with panel("Time Watch"):
        ...
        with c3:
            st.metric(
                "Late Work %", f"{float(sla_snapshot.get('breach_rate_pct', 0.0)):.1f}%"
            )
        ...
        with left:
            ...
            if not stages.empty:
                if "status" in stages.columns:
                    stages["status"] = stages["status"].map(
                        lambda x: status_display(str(x))
                    )
        ...
        with right:
            ...
            if not candidates.empty:
                if "status" in candidates.columns:
                    candidates["status"] = candidates["status"].map(
                        lambda x: status_display(str(x))
                    )
                show_cols = [
                    "shipment_id",
...
def render_ops_health_panel(
    service_summary: Dict[str, object], ops_health: Dict[str, object]
) -> None:
    with panel("Ops Health"):
        ...
        with m2:
            st.metric(
                "Critical No Owner", int(ops_health.get("critical_unassigned", 0))
            )
            st.metric(
                "Avg Age (h)", float(ops_health.get("avg_unresolved_age_hours", 0.0))
            )
...
def render_service_readiness_panel(health_report: Dict[str, object]) -> None:
    service_meta = (
        health_report.get("service_meta", {}) if isinstance(health_report, dict) else {}
    )
    artifacts = (
        service_meta.get("artifacts", {}) if isinstance(service_meta, dict) else {}
    )
    report_contract = (
        service_meta.get("report_contract", {})
        if isinstance(service_meta, dict)
        else {}
    )
    review_pack = (
        service_meta.get("review_pack", {}) if isinstance(service_meta, dict) else {}
    )
    runtime_scorecard = (
        service_meta.get("runtime_scorecard", {})
        if isinstance(service_meta, dict)
        else {}
    )
    action_impact_board = (
        service_meta.get("action_impact_board", {})
        if isinstance(service_meta, dict)
        else {}
    )
    summary = (
        health_report.get("summary", {}) if isinstance(health_report, dict) else {}
    )
    review_flow = (
        service_meta.get("review_flow", []) if isinstance(service_meta, dict) else []
    )
    two_minute_review = (
        service_meta.get("two_minute_review", [])
        if isinstance(service_meta, dict)
        else []
    )
    operator_rules = (
        service_meta.get("operator_rules", []) if isinstance(service_meta, dict) else []
    )
    watchouts = (
        service_meta.get("watchouts", []) if isinstance(service_meta, dict) else []
    )
    stages = service_meta.get("stages", []) if isinstance(service_meta, dict) else []
    proof_assets = (
        service_meta.get("proof_assets", []) if isinstance(service_meta, dict) else []
    )
    ...
    with panel(
        ...
        with c1:
            st.metric(
                "Health", str(health_report.get("overall_status", "unknown")).upper()
            )
        with c2:
        ...
        if review_pack:
            ...
            c1, c2, c3 = st.columns(3)
            proof_bundle = (
                review_pack.get("proof_bundle", {})
                if isinstance(review_pack, dict)
                else {}
            )
            approval_gate = (
                review_pack.get("approval_gate", {})
                if isinstance(review_pack, dict)
                else {}
            )
            diagnostics = (
                health_report.get("diagnostics", {})
            ...
            with left_review:
                review_two_minute = (
                    review_pack.get("two_minute_review", [])
                    if isinstance(review_pack, dict)
                    else []
                )
            ...
            reviewer_fast_path = [
                ...
                (
                    "Control Tower",
                    "Validate queue parity and current actionability in the main worklist.",
                ),
                (
                    "Evidence Pack",
                    "Check output artifacts before exporting downstream claims.",
                ),
                (
                    "Audit Chain",
                    "Confirm downstream handoff only after audit integrity stays green.",
                ),
            ]
            ...
            failing_ids = (
                diagnostics.get("failing_check_ids", [])
                if isinstance(diagnostics, dict)
                else []
            )
            warning_ids = (
                diagnostics.get("warning_check_ids", [])
                if isinstance(diagnostics, dict)
                else []
            )
            focus_check = (
            ...
            next_action = str(
                diagnostics.get("next_action", "Review worklist and evidence pack.")
            ).strip()
            route_lines = []
            ...
            if route_lines:
                st.markdown("**Focused Verification Route**")
                ...
                snapshot_lines = [
                    "the-logistics-prophet focused status snapshot",
                ...
                st.markdown("**Focused Verification Snapshot**")
        ...
        if runtime_scorecard:
            ...
            runtime_summary = (
                runtime_scorecard.get("summary", {})
                if isinstance(runtime_scorecard, dict)
                else {}
            )
            c1, c2, c3, c4 = st.columns(4)
            with c1:
            ...
            with c4:
                st.metric(
                    "Audit Checked", int(runtime_summary.get("audit_chain_checked", 0))
                )
        ...
        if action_impact_board:
            ...
            with c2:
                st.metric(
                    "ETA Gain (hrs)", int(impact_summary.get("eta_gain_hours", 0))
                )
            with c3:
                st.metric("Tracked KPIs", int(impact_summary.get("tracked_kpis", 0)))

            kpis = (
                action_impact_board.get("kpis", [])
                if isinstance(action_impact_board, dict)
                else []
            )
            if kpis:
...
def render_model_selection_panel(
    training: Dict[str, object], model_comparison: Dict[str, object]
) -> None:
    with panel("Model Selection"):
        st.caption(
            str(
                training.get("selection_reason", "")
                or model_comparison.get("selection_reason", "")
                or ""
            ).strip()
        )

        selected = str(
            training.get("selected_model", "")
            or model_comparison.get("selected_model", "")
            or ""
        ).strip()
        threshold = training.get(
            "selected_threshold", model_comparison.get("selected_threshold", "")
        )
        ...
        with c2:
            st.metric(
                "Threshold", _format_number(threshold, 3) if threshold != "" else "-"
            )
        with c3:
            st.metric(
                "AUC (Test)",
                _format_number(
                    (training.get("test_metrics", {}) or {}).get("auc", ""), 4
                ),
            )
        ...
        if isinstance(test_metrics, dict) and test_metrics:
            df = (
                pd.DataFrame.from_dict(test_metrics, orient="index")
                .reset_index()
...
def render_queue_panel(queue: pd.DataFrame) -> pd.DataFrame:
    ...
    owners = ["All"] + sorted(
        [owner for owner in queue["owner"].fillna("").unique().tolist() if owner]
    )
    ...
    with c2:
        status = st.selectbox(
            ...
            statuses,
            index=0,
            format_func=lambda s: "All" if s == "All" else status_display(str(s)),
        )
    with c3:
    ...
    with c4:
        top_n = st.slider(
            "Rows",
            min_value=10,
            max_value=min(150, len(queue)),
            value=min(40, len(queue)),
            step=10,
        )
    ...
    with panel("Queue"):
        ...
        if "risk_score" in show.columns:
            show["risk_score"] = pd.to_numeric(
                show["risk_score"], errors="coerce"
            ).fillna(0.0)
        ...
        try:
            picked_rows = list(
                getattr(getattr(selection, "selection", None), "rows", []) or []
            )
        except Exception:
            picked_rows = []
        if picked_rows:
            try:
                picked = str(
                    display.iloc[int(picked_rows[0])].get("Shipment", "") or ""
                ).strip()
            except Exception:
                picked = ""
            if picked:
        ...
        if inspect_options:
            ...
            if (
                current in inspect_options
                and st.session_state.get("queue_inspect") != current
            ):
                st.session_state["queue_inspect"] = current
            inspect_idx = (
                inspect_options.index(current) if current in inspect_options else 0
            )
...
def render_execution_console(queue: pd.DataFrame, user: Dict[str, object]) -> None:
    with panel("Update"):
        ...
        if (
            current in ship_options
            and st.session_state.get("update_pick_shipment") != current
        ):
            st.session_state["update_pick_shipment"] = current
        selected = st.selectbox(
            "Shipment", options=ship_options, key="update_pick_shipment"
        )
        set_selected_shipment(selected)
        ...
        status_index = (
            allowed_options.index(current_status)
            if current_status in allowed_options
            else 0
        )
        bulk_status_index = (
        ...
        with st.form("action_form", clear_on_submit=False):
            ...
            with c1:
                ...
                owner = st.text_input(
                    "Owner",
                    value=str(selected_row.get("owner", "")),
                    disabled=not can_update,
                )
            with c2:
                eta = st.text_input(
        ...
        if can_update:
            with st.form("bulk_action_form", clear_on_submit=False):
                ...
                bulk_shipments = st.multiselect(
                    "Shipments", options=queue["shipment_id"].tolist()
                )
                c1, c2 = st.columns(2)
                with c1:
                    ...
                    bulk_owner = st.text_input(
                        "Bulk Owner", value=str(selected_row.get("owner", ""))
                    )
                with c2:
            ...
            if bulk_submit:
                if not bulk_shipments:
                    ...
                    try:
                        ...
                        st.success(
                            f"Bulk updated {result.get('updated', 0)} shipments."
                        )
                        missing = result.get("missing", [])
                        invalid_transitions = result.get("invalid_transitions", [])
                        if missing:
                            st.warning(
                                f"Missing shipments skipped: {', '.join(missing[:5])}"
                            )
                        if invalid_transitions:
                            st.warning(
                                f"Skipped {len(invalid_transitions)} rows due to status flow rules."
                            )
                            invalid_df = pd.DataFrame(invalid_transitions).head(12)
                            if "current_status" in invalid_df.columns:
                                invalid_df["current_status"] = invalid_df[
                                    "current_status"
                                ].map(lambda x: status_display(str(x)))
                            if "requested_status" in invalid_df.columns:
                                invalid_df["requested_status"] = invalid_df[
                                    "requested_status"
                                ].map(lambda x: status_display(str(x)))
                            st.dataframe(
                                invalid_df, use_container_width=True, height=180
                            )
                ...
                [
                    {
                        ...
                        "payload": json.dumps(
                            row.get("payload", {}), ensure_ascii=True
                        ),
                    }
                    for row in activity
                ]
    ...
    *,
    ...
    if shap_row:
        ...
        for idx in [1, 2, 3]:
            ...
            lines.append(
                f"- {feat}: value={value} contribution={_format_number(contrib, 4)}"
            )
        lines.append("")
    ...
    if evidence:
        ...
        events = (
            evidence.get("delay_events", [])
            if isinstance(evidence.get("delay_events", []), list)
            else []
        )
        if events:
            lines.append("")
            ...
            for event in events[:8]:
                if not isinstance(event, dict):
                    continue
                lines.append(
                    f"- {event.get('cause', '')} ({event.get('severity', '')})"
                )
        lines.append("")
    ...
    lines.append(
        "- Confirm the driver signals with dashboards (latency, carrier performance, warehouse load)."
    )
    lines.append(
        "- Assign an owner and move the item to the next step with a clear ETA and note."
    )
    lines.append(
        "- Escalate to an incident if the pattern affects multiple critical items."
    )
    lines.append("")
    return "\n".join(lines)
...
def render_shipment_explain_panel(
    *,
    ...
    with panel("Explain"):
        ...
        if not shipment_id:
            st.caption(
                "Pick a shipment from Worklist / Queue / Update to see explainability and evidence."
            )
            return
        ...
        if can_update:
            st.caption(
                "Evidence next to action: validate drivers, then execute the next step with an auditable change."
            )
            ...
            st.caption(
                "Role is read-only for queue updates; explainability is available but actions are disabled."
            )
        ...
        if shap_row:
                ...
                driver_rows.append(
                    {
                        ...
                        "value": feature_row.get(feat, ""),
                        "contribution": float(
                            shap_row.get(f"contribution_{idx}", 0.0) or 0.0
                        ),
                    }
                )
            ...
            if driver_rows:
                ...
                fig = go.Figure()
                colors = [
                    "#0f625b" if v >= 0 else "#ad5e4a"
                    for v in df["contribution"].tolist()
                ]
                fig.add_trace(
                    go.Bar(
                        x=df["contribution"],
                ...
                fig.update_layout(
                    ...
                    plot_bgcolor="rgba(0,0,0,0)",
                    xaxis=dict(
                        showgrid=True, gridcolor="rgba(0,0,0,.08)", title="Contribution"
                    ),
                    yaxis=dict(showgrid=False, title=""),
                )
                ...
                show["contribution"] = show["contribution"].map(
                    lambda v: _format_number(v, 4)
                )
                st.dataframe(show, use_container_width=True, height=160)
        else:
            st.caption(
                "No local SHAP row found for this shipment. Re-run the pipeline to refresh explanations."
            )
        ...
        if can_incident:
            risk_band = str(
                queue_row.get("risk_band", "") or evidence.get("risk_band", "") or "Low"
            )
    ...
    with panel("Incident"):
        ...
        with st.form("incident_form", clear_on_submit=True):
            ...
            description = st.text_area(
                "Description", height=90, disabled=not can_manage
            )
        ...
        recommendations = derive_incident_recommendations(
            path=SERVICE_DB_PATH, max_items=5
        )
        ...
        if recommendations:
            ...
            if can_manage:
                ...
                if st.button("Create Incident", use_container_width=True):
                    ...
                    if bool(result.get("deduplicated")):
                        st.success(
                            f"Incident updated (deduplicated): {result.get('incident_id')}"
                        )
                    else:
                        st.success(
                            f"Incident created from recommendation: {result.get('incident_id')}"
                        )
                    st.rerun()
        else:
...
def fetch_activity_window(
    *, start_utc: str, end_utc: str, limit: int = 200
...
def _build_postmortem_draft(
    incident: Dict[str, object], timeline_rows: List[Dict[str, object]]
) -> str:
...
def render_postmortem_export_panel(user: Dict[str, object]) -> None:
    with panel("Postmortem Export"):
        ...
        selected_label = st.selectbox(
            "Select Incident", options=labels, key="pm_incident_pick"
        )
        idx = labels.index(selected_label)
        incident = incidents[idx]

        max_events = st.slider(
            "Timeline events", min_value=20, max_value=300, value=120, step=20
        )
        ...
        start_utc = (
            opened_at or (datetime.now(timezone.utc) - timedelta(hours=48)).isoformat()
        )

        activity_rows = fetch_activity_window(
            start_utc=start_utc, end_utc=end_utc, limit=max_events
        )
...
def render_evidence_pack_panel(user: Dict[str, object]) -> None:
    with panel(
        "Evidence Pack",
        caption="Build a verifiable ZIP with key artifacts + integrity manifest (offline).",
    ):
        include_graph = st.checkbox(
            "Include RDF instance graph (TTL)", value=True, key="pack_include_graph"
        )
        include_ops = st.checkbox(
            "Include ops report (HTML)", value=True, key="pack_include_ops"
        )
        ...
        if st.button("Build Evidence Pack", use_container_width=True):
            try:
                ...
                st.success(
                    f"Evidence pack built: {filename} ({len(data) / 1024 / 1024:.2f} MB)"
                )
            except Exception as exc:  # noqa: BLE001
        ...
        if st.session_state.get("evidence_pack_bytes"):
            st.download_button(
                ...
                file_name=str(
                    st.session_state.get("evidence_pack_filename")
                    or "evidence-pack.zip"
                ),
...
def render_governance_overview_panel() -> None:
    with panel("Governance"):
        ...
        with c4:
            st.caption(
                f"Latest hash: `{str(latest)[:18]}...`" if latest else "Latest hash: -"
            )
        ...
        if not recent.empty:
            ...
            if "created_at" in show.columns:
                show.insert(
                    0, "age", show["created_at"].map(lambda x: _age_text(str(x)))
                )
...
def render_community_hub_panel(user: Dict[str, object]) -> None:
    with panel(
        "Community Hub", caption="Optional integrations: Formspree + Disqus + Giscus"
    ):
        endpoint = str(os.getenv("LP_FORMSPREE_ENDPOINT", "")).strip()
        disqus_shortname = str(os.getenv("LP_DISQUS_SHORTNAME", "")).strip()
        disqus_identifier = str(
            os.getenv("LP_DISQUS_IDENTIFIER", "the-logistics-prophet-governance")
        ).strip()
        page_url = str(
            os.getenv(
                "LP_COMMUNITY_PAGE_URL",
        ...
        giscus_mapping = (
            str(os.getenv("LP_GISCUS_MAPPING", "specific")).strip() or "specific"
        )
        giscus_term = (
            str(os.getenv("LP_GISCUS_TERM", disqus_identifier)).strip()
            or disqus_identifier
        )
                ...
                if not clean_message:
                    ...
                    try:
                        ...
                        if resp.status_code >= 400:
                            detail = str(
                                response_payload.get("error") or "request failed"
                            )
...
4. Use **Incidents** when suggestions trigger or risk grows.
    ...
    if page == "Queue + Update":
        ...
        render_shipment_explain_panel(
            shipment_id=get_selected_shipment(
                filtered if not filtered.empty else queue
            ),
            queue=queue,
            shap_local=shap_local,
            graph=rdf_graph,
