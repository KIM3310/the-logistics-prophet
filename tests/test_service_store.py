...
class TestServiceStore(unittest.TestCase):
    ...
    def test_action_update_writes_activity(self) -> None:
        ...
        activity = fetch_activity(
            shipment_id=shipment_id, path=SERVICE_DB_PATH, limit=5
        )
    ...
    def test_bulk_update_and_ops_health(self) -> None:
        ...
        owners = {
            str(item.get("owner")): int(item.get("count", 0))
            for item in backlog
    ...
    def test_incident_recommendation_rules(self) -> None:
        ...
        recommendations = incident_recommendations_from_metrics(
            queue_summary=queue_summary, ops_health=ops_health, max_items=5
        )
    ...
    def test_incident_recommendation_deduplication(self) -> None:
        ...
        try:
            cnt = conn.execute(
                "SELECT COUNT(*) FROM service_incidents WHERE source_rule = ?",
                (unique_rule,),
            ).fetchone()[0]
        finally:
            conn.close()
