...
class TestAuthRbacAudit(unittest.TestCase):
    ...
    @staticmethod
    def _authenticate_with_candidates(
        username: str, candidates: list[str], *, path: Path
    ):
        ...
        with tempfile.TemporaryDirectory(prefix="logistics_audit_chain_reset_") as tmp:
            ...
            self.assertIsNotNone(
                self._authenticate_with_candidates(
                    "operator",
                    [
                        os.getenv("LP_DEMO_OPERATOR_PASSWORD", "OperatorOps!2026"),
                        "ops123!",
                    ],
                    path=db_path,
                )
    ...
    def test_auth_lockout_policy_enforced(self) -> None:
        with tempfile.TemporaryDirectory(prefix="logistics_auth_lock_") as tmp:
            ...
            try:
                ...
                first = authenticate_user_with_status(
                    "admin", "wrong-password", path=db_path
                )
                ...
                second = authenticate_user_with_status(
                    "admin", "wrong-password", path=db_path
                )
